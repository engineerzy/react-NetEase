...
const useTouch: UseTouch = (ref, wrapper, progress, initPercent) => {
	...
	useEffect(() => {
		...
		const handleTouchMove = (e: TouchEvent) => {
			if (e.touches[0].clientX < wrapperRect.current.left || e.touches[0].clientX > wrapperRect.current.right - 8) return;
			if (e.touches[0].clientX < wrapperRect.current.left) {
				setPercent(0)
			} else if (e.touches[0].clientX > wrapperRect.current.right - 4) {
				setPercent(1)
			} else {
				setPercent((e.touches[0].clientX - wrapperRect.current.left) / wrapperRect.current.width)
			}
		}
	...
	useEffect(() => {
		ref.current.style.left = `${(wrapperRect.current.width * initPercent).toFixed(2)}px`
		progress.current.style.width = `${(initPercent*wrapperRect.current.width + 4).toFixed(2)}px`
